...
export function ensureCanvasSuggestionBubble({
  ...
  bubble.innerHTML = `
    <div class="canvas-ai-window-title">鎺ヤ笅鏉ユ兂鍋氫粈涔堬紵</div>
    <div class="canvas-ai-actions">
      <button type="button" disabled><span>璇嗗埆涓?/span></button>
    </div>
  `;
...
export function renderCanvasSuggestionBubble({
  ...
  bubble.querySelector(".canvas-ai-actions").innerHTML = bubble._actions.map((action, index) => `
    <button type="button" data-canvas-ai-action="${index}">
      <span>${escapeHtml(String(action.title || "鐢熸垚").slice(0, 4))}</span>
    </button>
  `).join("");
}
