...
export function initTaskBar({
  ...
  if (bindPromptSubmit) promptForm?.addEventListener("submit", async (event) => {
    ...
    const selectedModel = resolveImageModelId(
    ...
    if (chatModelSelect && chatModelSelect.value !== selectedModel) {
      ...
      chatModelSelect.dataset.selectedModelId = selectedModel;
    ...
    try {
      ...
      let result = await postJson("/api/ai/generate", buildChatImagePayload({
        model: selectedModel,
        prompt,
        images,
      ...
      if (!result?.imageUrl && result?.jobId) {
        result = await waitForTaskbarGenerationJob(result.jobId, {
          fallback: result,
          onProgress: (payload) => {
...
async function waitForTaskbarGenerationJob(jobId, {
  attempts = 180,
  delayMs = 2000,
  fallback = {},
  onProgress = null,
  ...
  for (let index = 0; index < attempts; index += 1) {
    ...
    if (["succeeded", "failed", "cancelled", "timeout", "save_failed"].includes(payload?.status)) {
      ...
      const imageUrl = getTaskbarResultImageUrl(lastPayload);
      if (!imageUrl) {
        missingUrlAttempts += 1;
        if (missingUrlAttempts <= missingUrlRetries) {
          onProgress?.({ ...lastPayload, status: "running", message: "Waiting for saved image URL" });
          continue;
        }
...
function getTaskbarResultImageUrl(result = {}) {
