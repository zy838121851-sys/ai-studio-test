...
- `scripts/check-style-entry.js` now guards key selectors in
  ...
  `styles/features/node-generation.css`, and `styles/features/node.css` so
...
- `styles/features/node.css` now owns image generator node frame/panel styles,
