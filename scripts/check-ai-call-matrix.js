...
try {
  calls.length = 0;
  const doubao = await generateImage({ model: DEFAULT_IMAGE_MODEL, prompt: "home", size: "2K" });
  assert(doubao.providerCalls.map((call) => call.provider).join(",") === "volcengine", "Doubao generation should call only Volcengine");

  calls.length = 0;
  const wan = await generateImage({ model: "wan2.7-image-pro", prompt: "wan", size: "2K" });
  assert(wan.providerCalls.map((call) => call.provider).join(",") === "qwen", "Wan generation should call Qwen provider");
