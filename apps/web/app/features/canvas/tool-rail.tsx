...
export function CanvasToolRail({ onToolChange }: { onToolChange?: (tool: ToolId) => void }) {
  ...
  return (
    <aside className={`canvas-tool-rail${collapsed ? " is-collapsed" : ""}`} aria-label="Canvas tools" data-collapsed={collapsed}>
      ...
      {!collapsed ? <div className="canvas-tool-rail__items">{tools.map(({ id, label, icon: Icon }) => <div className="canvas-tool-rail__item" key={id}><button className={`rail-btn${activeTool === id ? " is-active" : ""}`} type="button" data-tool={id} aria-label={label} aria-expanded={openPanel === id} title={label} onClick={() => chooseTool(id)}><Icon size={20} aria-hidden="true" /></button>{openPanel === id ? <div className="canvas-tool-rail__submenu" role="menu" aria-label={`${label} options`}>{id === "shape" ? <button type="button" data-shape-tool="rectangle" role="menuitem" onClick={() => chooseTool("shape")}>Rectangle</button> : null}{id === "pen" ? <button type="button" data-pen-tool="pen" role="menuitem" onClick={() => chooseTool("pen")}>Pen</button> : null}</div> : null}</div>)}</div> : null}
    </aside>
  );
}
