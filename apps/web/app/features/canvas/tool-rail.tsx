...
export function CanvasToolRail({
  ...
  onShapeChange?: (shape: ShapeType | "arrow") => void;
}) {
  ...
  return (
    <aside
      className={`canvas-tool-rail${collapsed ? " is-collapsed" : ""}`}
      aria-label="Canvas tools"
      data-collapsed={collapsed}
    >
      <button
        className="canvas-tool-rail__collapse"
        type="button"
      ...
      {!collapsed ? (
        <div className="canvas-tool-rail__items">
          {tools.map(({ id, label, icon: Icon }) => (
            <div className="canvas-tool-rail__item" key={id}>
              <button
                className={`rail-btn${activeTool === id ? " is-active" : ""}`}
                type="button"
                data-tool={id}
                aria-label={label}
              ...
              {openPanel === id ? (
                <div
                  className="canvas-tool-rail__submenu"
                  role="menu"
